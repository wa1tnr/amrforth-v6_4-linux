...
\ over-sampled to get 14 bits of date.
...
code adc@  (  - n)
	begin  adc-flag A mov  0<> until
	IE push  \ Save the interrupts flag.
	$7f # IE anl  \ Disable interrupts globally.
	total1 A mov  A R4 mov
	total2 A mov  A R5 mov
	total3 A mov  A R6 mov
...
code IE@  (  - c)  IE A mov  Apush  A clr  Apush  next c;
code EIE2@  (  - c)  EIE2 A mov  Apush  A clr  Apush  next c;
code adc-flag@  (  - c) adc-flag A mov  Apush  A clr  Apush  next c;
code adc-counter@  (  - c)
	adc-counter A mov  Apush  A clr  Apush  next c;
code total@  (  - ud)
	$7f # IE anl
	acc1 A mov  Apush
	acc2 A mov  Apush
