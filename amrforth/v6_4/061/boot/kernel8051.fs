...
sfr-file s" sfr-31.fs"   $= [if]  create 80c31   [then]
sfr-file s" sfr-32.fs"   $= [if]  create 80c32   [then]
...
in-assembler rom-start dup cr .( ROM-START=) u. cr
romming [if] $7fff and [then]  \ For amr552.
org in-meta
...
a: |flip  R2 A xch ;a
\ i: flip  [[ in-assembler ]]  |flip  [[ in-immed ]] ;i
s" flip" kernel-entry  \ For single stepper.
...
a: |2+  ( n1 - n2) C clr  2 # A add  +C if  R2 inc  then ;a
...
\ e.g.  [asm  $ff #and ]
a: #and  ( n - ) dup # A anl  8 rshift # 'R2 anl ;a
a: #or   ( n - ) dup # A orl  8 rshift # 'R2 orl ;a
a: #xor  ( n - ) dup # A xrl  8 rshift # 'R2 xrl ;a
a: #lit  ( n - ) dup # A mov  8 rshift # R2 mov ;a
...
f: number-t   ( a - n)  \ 32 bit to 16 bit
    number double? if
        ...
        drop
    then  ( stepper-source-entry) literal-t ;f
...
m: :  (  - ) !csp tcreate target definitions in-meta ] ;m
...
m: cpuCREATE  (  - ) to-meta ( precreate) logIRAM cpuDP @ constant ;m
...
s" cfetchp" kernel-entry  \ For single stepper.
code c@p  ( a - c)
...
s" minus" kernel-entry  \ For single stepper.
code -  ( n1 n2 - n3)
...
: =  ( n1 n2 - flag) xor -;
code 0=  ( n - flag) c;
s" not" kernel-entry  \ For single stepper.
code not  ( n - flag)  \ Logical, not bitwise.
    |0=  next c;

0 [if]  \ Changed 02Jul2004, Charley.
\ : u>  ( n1 n2 - flag) swap -;
\ code u<  ( n1 n2 - flag)
\     A DPL mov  R2 DPH mov  ' drop call
\     R2 A xch  DPH A = if  R2 A xch  DPL A = if then then
\     ACC A subb  A R2 mov  next c;
[then]
\ Faster and smaller with no jumps.
: u>  ( n1 n2 - flag) swap -;
code u<  ( u1 u2 - flag)
...
\ : >  ( n1 n2 - flag) swap -;
\ code <  ( n1 n2 - flag)
\     A DPL mov  R2 DPH mov  ' drop call  A R1 mov
\     R2 A mov  DPH A xrl  7 .ACC set? if
\ \        R1 A mov  \ Fixed 11Mar2004, Charley.
\ 	R2 A mov
\     else
\         C clr  R1 A mov  DPL A subb  R2 A mov  DPH A subb
\     then
\     7 .ACC C mov  ACC A subb  A R2 mov
\     next c;
[then]
\ Faster and smaller with no jumps.
:  >  ( n1 n2 - flag) swap -;
...
s" push" kernel-entry  \ For single stepper.
\ code push  c;
code (>r)  ( n - )
...
s" pop" kernel-entry  \ For single stepper.
\ code pop  c;
code (r>)  (  - n)
...
i: else  ( flag1 a1 - flag2 a2) $02 c,-t ?>mark 2>r ?>resolve 2r> ;i
i: then  ( flag a - ) ?>resolve hide ;i
i: begin  (  - flag a) ?<mark hide ;i
i: until  ( flag a - ) [t'] ?branch token, ?<resolve ;i
i: again  ( flag a - )
    swap ?condition stepper-source-entry [[ in-assembler ]] jump ;a
i: while  (  - flag a) if ;i
...
i: 1-dup0=until  ( flag a - )
    swap ?condition [[ in-assembler ]] ACC -zero until  hide ;a
i: dupif  (  - a1 flag a2)
    [[ in-assembler ]] 0 # A = if  0 # R2 = if  $80 if  
    fswap then fswap then  -2 over ;a
i: dup0=if  (  - a1 flag a2)
    [[ in-assembler ]] 0 # A = if  -2  0 # R2 = if  hide ;a
i: dup0<if  (  - a1 flag a2)
    [[ in-assembler ]] R2 B mov  7 .B set? if  -2 over ;a
i: dup0<notif  (  - a1 flag a2)
    [[ in-assembler ]] R2 B mov  7 .B clr? if  -2 over ;a
i: dupuntil  ( flag a - )
    swap ?condition [[ in-assembler ]]
...
i: dup>r  ( n - n) [[ in-assembler ]] ACC push  'R2 push ;a
i: r>drop  (  - ) [[ in-assembler ]] RP dec  RP dec ;a
...
s" emit" kernel-entry  \ For single stepper.
code emit  ( c - )
...
i: exec:  [t'] (exec:) token, true to table? ;i
