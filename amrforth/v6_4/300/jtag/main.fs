...
: read-idcode  (  - d)
	...
	shift-DR
\	0 15 for  $8000 Out K  next
\	0 14 for  $8000 Out K  next  $8000 Out
...
: write-8-bits  ( c - )
	shift-DR
\	7 for  In K  next  drop  \ 8 bits of data.
	8 for  In K  next  drop  \ 8 bits of data.
	3 In K In drop  \ Write opcode.
\	4 In K In drop  \ Write opcode.
	run-test/idle ;

: write-16-bits  ( n - )
	shift-DR
\	15 for  In K  next  drop  \ 16 bits of data.
...
: remoteC@  ( addr - c) load-address read-byte ;

: remote-dump  ( addr len - )
	halt 
	swap load-address
\	16 /mod swap 0= if  1 -  then  \ ***** is 1 - correct?
	16 /mod swap if  1 +  then
\	for  cr 15 for  read-byte hb.  next next
	for  cr 16 for  read-byte hb.  next next
	;
' remote-dump header dump

\ : dump-line  (  - ) cr 15 for  read-byte hb.  next ;
: dump-line  (  - ) cr 16 for  read-byte hb.  next ;
...
: local-show  ( block# - )
\	block [ 512 16 / 1 - ] literal for
	block [ 512 16 / ] literal for
\		cr 15 for  count hb.  next
...
: remoteC!  ( c addr - ) load-address write-byte ;
...
\ Move a 512 byte page from target into host.
\ : get  ( page block - )
\	swap page load-address block
\	dup local-erase 511 for
\		read-byte over localC! 1 +
\	next  drop ;
\ ' get header get

: delay  ( n - ) for next ;

\ Move a 512 byte block from host into target.
\ : put  ( block page - )
\	page remote-erase block
\	511 for
\		dup localC@ write-byte 1 +
\	next drop ;
\ ' put header put
...
\ : fill-buffer  (  - ) buffer addr! 63 for  key !+  next ;
: fill-buffer  (  - ) buffer addr! 64 for  key !+  next ;

: write-buffer  (  - )
\    buffer addr! 63 for  @+ write-byte  next ;
    buffer addr! 64 for  @+ write-byte  next ;

\ : write-page  (  - ) 7 for  fill-buffer write-buffer  0 emit  next ;
...
\ : C2id@  (  - c) C2reset 1 ms 0 C2a! 25 us C2d@ ;
...
code split  ( a - c1 c2)
	...
	' dup call  R7 A mov
	next c;
\    Apop  A R7 mov  A clr  Apush
\    R7 A mov  Apush  A clr  Apush
\	next c;
...
\ : C2dump1  (  - ) 1 dump-page c!  256 h. 256 C2dump ;
\ ' C2dump1 header C2next
...
: C2write-buffer  ( addr - )
	...
	C2data@ $0d - if  .? exit  then
\	buffer addr! 63 for  @+ C2data! ( 10 us)  next
	buffer addr! 64 for  @+ C2data! ( 10 us)  next
	poll-OutReady ;

: C2write-page  ( addr1 - addr2)
\	7 for	fill-buffer dup C2write-buffer 64 + 0 emit
...
\	0 key 1 - for  C2write-page  next  drop ;
